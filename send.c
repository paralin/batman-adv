...
static unsigned long own_ogm_send_time(const struct bat_priv *bat_priv)
{
			      ...
			      struct hard_iface *hard_iface)
{
	...
	while (aggregated_packet(buff_pos,
		...
		bat_dbg(DBG_BATMAN, bat_priv,
			...
			(ogm_packet->flags & DIRECTLINK ?
			 "on" : "off"),
			ogm_packet->ttvn, hard_iface->net_dev->name,
			hard_iface->net_dev->dev_addr);

		buff_pos += sizeof(*ogm_packet) +
			tt_len(ogm_packet->tt_num_changes);
		packet_num++;
				  ...
				  struct hard_iface *hard_iface)
{
	...
	ogm_packet = (struct ogm_packet *)hard_iface->packet_buff;
	...
	ogm_packet->tt_num_changes = tt_changes_fill_buffer(bat_priv,
				hard_iface->packet_buff + BAT_PACKET_OGM_LEN,
				hard_iface->packet_len - BAT_PACKET_OGM_LEN);
	...
	struct hard_iface *hard_iface)
{
	...
	realloc_packet_buffer(hard_iface, BAT_PACKET_OGM_LEN);

	ogm_packet = (struct ogm_packet *)hard_iface->packet_buff;
	ogm_packet->tt_num_changes = 0;
}
...
void schedule_own_ogm_packet(struct hard_iface *hard_iface)
{
	...
	ogm_packet->ttvn = atomic_read(&bat_priv->ttvn);
	ogm_packet->tt_crc = htons((uint16_t)atomic_read(&bat_priv->tt_crc));
			     ...
			     struct hard_iface *if_incoming)
{
	...
	tt_num_changes = ogm_packet->tt_num_changes;
	...
	if (router && router->tq_avg != 0) {
		...
		if (!compare_eth(router->addr, ethhdr->h_source)) {
			ogm_packet->tq = router->tq_avg;

			if (router->last_ttl)
				ogm_packet->ttl = router->last_ttl - 1;
		}
	...
	ogm_packet->tt_crc = htons(ogm_packet->tt_crc);
	...
	add_bat_packet_to_list(bat_priv,
			       ...
			       sizeof(*ogm_packet) + tt_len(tt_num_changes),
