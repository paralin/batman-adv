...
static void batadv_bw_start_timer(struct batadv_bw_vars *bw_vars);
				   ...
				   struct batadv_bw_vars *bw_vars)
{
	...
	while (1) {
		...
		window_end = min(bw_vars->window_first + BW_WINDOW_SIZE,
...
static void batadv_bw_worker(struct work_struct *work)
{
	...
	printk ("I have been called!! bw_vars->last_sent_time %lu\n", bw_vars->last_sent_time);
	return;
	...
	printk("if %u < %lu\n", bw_vars->window_first, bw_vars->total_to_send -1);
